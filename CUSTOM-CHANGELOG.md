...
| 2026-05-20 | `codi-v4.14.0-fazer-ai.74`      | `v4.14.0-fazer-ai.74`       | **4.14.0**    | First Chatwoot core bump since `.6` (4.13 → 4.14). New branch off the upstream tag; merged 159 commits of customizations from `codi-v4.13.0-fazer-ai.66`. Conflicts resolved: `Gemfile.lock`, `app/builders/account_builder.rb` (kept our `default_account_locale` pt_BR + added upstream `custom_attributes` onboarding_step), `app/javascript/dashboard/components/widgets/conversation/ConversationHeader.vue` (kept our `PropriacloudStatusBadge` + upstream snooze separator), `app/javascript/dashboard/featureFlags.js` (dropped legacy `QUOTED_EMAIL_REPLY` per upstream removal, kept our channel flags), `config/features.yml` (dropped `chatwoot_internal: true` on `companies` per upstream, kept our channel entries), `db/schema.rb` (took upstream version `2026_05_15_000000` + companies columns). Upstream WhatsApp fixes absorbed: presence_subscribe_bulk throttle, Baileys disconnect fail-open, Embedded Signup waba_id fix + convert-to-cloud surface. No new branding drift introduced. Safety tag: `codi-pre-upgrade-2026-05-20`. |
| 2026-06-28 | `codi-v4.15.1-fazer-ai.86`      | `v4.15.1-fazer-ai.86`       | **4.15.1**    | Minor core bump (4.14 → 4.15). New branch cut **from current `codi-v4.14.0-fazer-ai.74`** (base = our codi → customizations stay `--first-parent`), then upstream tag `v4.15.1-fazer-ai.86` merged IN — 178 upstream commits. 11 conflicts (KC/AI/CO): `config/features.yml` (kept `channel_whatsapp_propriacloud` at bit 3, appended `conversation_unread_counts` at END to preserve FlagShihTzu indices), `app/models/channel/whatsapp.rb` (kept merge-semantics `update_provider_connection!` + layered upstream no-op guard / `Inbox.no_touching` / broadcast + new reachout/new-chat-cap methods), `sessions_controller.rb` (our `check_default_auth_disabled` + upstream session-limit suite), `login/Index.vue`·`MessagesView.vue`·`ChannelItem.vue`·`ConversationHeader.vue` (unions of our propriacloud UI + upstream reachout/message-cap + `whatsapp_call`), `es/inboxMgmt.json`, `package.json`/`pnpm-lock.yaml` (upstream vite 6.4.2), `enterprise/app/models/company.rb` (annotation). Auto-merge semantic sweep clean (4.15.0 `EmojiInput` rename, moved `FloatingCallWidget`, deleted `token_validation_service`). Post-merge `Gemfile.lock` repair: auto-merge had dropped the `omniauth_openid_connect` tree + violated the `vite_ruby ~> 3.7.0` pin (`31c1bd51`). Verified by a 4-lens read-only subagent panel + live boot (`/health` → `propriacloud` 4.15.1). Safety tag: `codi-pre-upgrade-2026-06-27`. |
| 2026-07-18 | `codi-v4.15.1-fazer-ai.88`      | `v4.15.1-fazer-ai.88`       | 4.15.1        | Iteration bump only — Chatwoot core unchanged. Branch cut **from `codi-v4.15.1-fazer-ai.86`** (base = our codi), upstream tag `.88` merged IN — 22 commits / 36 files / +995−17. `.87` skipped (cumulative, contained in `.88`); `.88` == `fazerai/main` HEAD. Chatwoot core `v4.16.0` was already tagged upstream but fazer-ai had not adopted it — core releases are not our trigger. 4 conflicts, all additive unions (no customization dropped). Post-merge fixes: corrected the remote topology upstream imported into `AGENTS.md`, gated the new session-import CTA off propriacloud inboxes. Safety tag: `codi-pre-upgrade-2026-07-18`. **Current branch.** |
...
### 2026-07-19 (WhatsApp groups for the Própria Cloud provider + OpenAPI v1.4.0 spec-drift fixes)

- **fix(whatsapp): propriacloud spec-drift fixes from the OpenAPI v1.4.0 audit** — an exhaustive cross-reference of the whatsapp-api `docs/openapi.yaml` (v1.4.0) against the `propriacloud` integration surfaced several real bugs, now fixed with regression specs: (1) **typing indicators + read receipts were sent to an empty JID** — `toggle_typing_status` and `read_messages` built their body with `jid_param(phone_number)`, but `Channel::Whatsapp` dispatches only `recipient_id:`, so the local `phone_number` kwarg was always `nil` and every `/presence/chat` and `/messages/mark-read` targeted `{user: ''}` (100% non-functional presence/blue-ticks); switched to the resolved `@phone_number`. (2) **QR self-heal retry hit a GET-only endpoint with POST** — `pair_qrcode`'s 404-rebuild path re-issued `/instances/pair/qrcode` as a POST (the spec defines only GET, `operationId getQRCode`), so the self-heal always failed; changed to GET. (3) **WABA delete returned 428 unhandled** — `disconnect_channel_provider` treated only 404 as success, so a still-paired WABA instance (spec returns `428 Precondition Required`) raised and, via the error wrapper, risked re-creating the instance mid-delete; now unpairs then retries the delete on 428. (4) **group `topic` clobbered a user-set `description`** — `sync_group` mapped `GroupInfoResponse.topic` onto `additional_attributes['description']`, so every `group.info_changed` reconcile overwrote the long description with the short topic (the spec keeps them distinct and never reads `description` back); topic is now stored under its own key and `description` is left untouched. New spec file `whatsapp_propriacloud_service_drift_spec.rb` (5 examples). Remaining audit findings (on-demand `request_chat_history` result ingestion, `message.receipt` Web-mode receipt-shape handler, quoted-reply `contextInfo` nesting, `message.echo` for WABA coexistence) are logged for follow-up.
- **fix(whatsapp): contact backfill no longer mints fabricated `+<lid>` contacts** — `HistoryBackfillService#backfill_contacts` derived the phone from each `/sync/contacts` row's primary `jid`, but the spec's `SyncContact` carries a LID in `jid` for `source=lid_mapping` rows (that's why it also ships dedicated `phone_number`, `pn_jid` and `lid_jid` fields). Deriving phone from a LID minted a `source_id=<lid-digits>` / `phone_number=+<lid-digits>` contact; when a real message later arrived with the person's true phone digits as `source_id`, a SECOND distinct contact was created — a duplicate plus a junk `+<lid>` record. Now resolves phone from `phone_number` → `pn_jid` → a phone-shaped `jid`, and skips rows that resolve to no phone JID. Regression covered in `history_backfill_service_spec.rb`. (A pre-existing quirk noted during this work: `each_page` has no page cap for `/sync/contacts` / `/sync/push-names`, so a mocked endpoint that never returns an empty page loops — see the hanging `PEND-13` fixture; harmless against a well-behaved API but worth a cap.)
- **fix(whatsapp): label the Própria Cloud provider card "WhatsApp", not "WhatsApp Web"** — the propriacloud provider supports both Web (QR / phone-code pairing) and API/WABA modes, so the new-inbox provider-selection card's `INBOX_MGMT.ADD.WHATSAPP.PROVIDERS.PROPRIACLOUD_DESC` subtitle "WhatsApp Web" was inaccurate; renamed to "WhatsApp" across `en` / `pt_BR` / `es` (a brand string, kept consistent across the shipped locales). The QR pairing-method label ("WhatsApp Web (Pairing)" / "(Emparelhamento)") is deliberately left as-is — it correctly names Web pairing specifically.
- **feat(whatsapp): WhatsApp group conversations + management for `propriacloud`** — the `propriacloud` provider previously dropped every `@g.us` message (`messages_upsert.rb` returned early for `jid_type == 'group'`), so group chats never reached Chatwoot even though the baileys provider had full group support. Wired propriacloud into the same channel-agnostic group model the fork already ships (`Contact`/`Conversation#group_type`, `GroupMember`, the `GroupConversationHandler` concern) so **no new frontend was needed** — the existing provider-agnostic group panel/API light up once the feature flag is on. Gated by `PROPRIACLOUD_WHATSAPP_GROUPS_ENABLED` (DB-config cascade → env, mirrors `BAILEYS_WHATSAPP_GROUPS_ENABLED`; `dashboard_controller.rb` OR-s propriacloud into the frontend flag). Implemented the full `base_service` group interface on `WhatsappPropriacloudService` mapped onto the whatsapp-api `/groups/*` surface (create, participants add/remove/promote/demote, name/description/photo, invite-link get+revoke, announce/locked, join-approval, member-add-mode, pending join-requests, leave) plus `sync_group` (roster + settings reconcile via `GET /groups/info`). Inbound `@g.us` messages become group conversations via a new `PropriacloudHandlers::GroupMessage` concern (sender attributed to `key.participant`, not the group); `group.joined` / `group.info_changed` webhooks drive membership reconciliation through a new `GroupMessage`/`GroupUpdate` handler pair (whatsapp-api emits no per-participant events, so `/groups/info` is the source of truth). Group replies route to `server: g.us` via a server-aware `jid_param` and a group-aware `SendOnWhatsappService#recipient_id`. JID modeling: `Contact#identifier` = full `<id>@g.us`, `ContactInbox#source_id` = bare id (the WhatsApp `source_id` regex rejects `@g.us`). Provider errors now include a shared `Whatsapp::Providers::GroupOperationError` marker (baileys errors too) so the group controllers render friendly 422s regardless of provider. Fixed a latent `jid_param` bug (device suffix `<phone>:<n>` digits were being merged into the user part). New specs: `whatsapp_propriacloud_service_groups_spec.rb` (14) + `group_message_spec.rb` (4), all green; zero regressions in the existing propriacloud suite. Docs: `CUSTOM-WHATSAPP-API.md` §Groups, `.env.example` + `.env`.

### 2026-07-18 (Upstream `.88` iteration bump — merge + fork-topology repair + upgrade-guide hardening)

- **Merge upstream `v4.15.1-fazer-ai.88`** (`f4de4b18`) — new branch `codi-v4.15.1-fazer-ai.88` cut **from `codi-v4.15.1-fazer-ai.86`** (base = our codi, per the standardized direction) with the upstream tag merged INTO it. 22 upstream commits, 36 files. Chatwoot core stays 4.15.1 — this is a fazer-ai iteration bump only; `.87` was skipped because the tags are cumulative points on the same `main` line and `.88` contains it. Upstream had already tagged Chatwoot core `v4.16.0`, but fazer-ai has not adopted it, so it is out of scope. Incoming features: widget redirect token for pre-authenticated deep links (new `Widget::RedirectToken` service + account/widget controllers), WhatsApp Web session import via browser extension (Baileys-only), `macro.executed` webhook now carries macro + executor, WhatsApp template media URL auto-fill from the template's example handle, internal-chat user-deletion FK repair (`on_delete: :nullify` on channels' `created_by_id`, `:cascade` on drafts), and Chrome Web Store extension link fix. **4 conflicts, all additive unions — no customization dropped**: `inboxes_controller.rb` (`check_authorization` `except:` list = union of our `refresh_provider_status` / `audit_stream` with upstream's `import_whatsapp_session`, wrapped to stay inside the 150-char limit); `{en,pt_BR}/inboxMgmt.json` (our `PAIRING_METHOD_*` / `QR_INSTRUCTIONS` / `PAIR_LOCK_COUNTDOWN` / `ALREADY_PAIRED` block kept alongside upstream's four new `IMPORT_SESSION_*` keys, both locales symmetric); `WhatsappLinkDeviceModal.vue` (our per-tab pairing-mode state kept, upstream's `showImportDetails` ref added). Migration `20260703000000` applied. Rebrand sweep found **zero** new must-translate drift, and all 15 watchlist files verified intact. Verified by a 37-agent read-only audit (6 dimensions, every finding adversarially re-checked) plus live boot (`/health` → `{"status":"woot","platform":"propriacloud","version":"4.15.1"}`), 302 rspec examples and 41 vitest tests. Safety tag: `codi-pre-upgrade-2026-07-18`.
- **docs(agents): correct fork remote topology inherited from upstream** (`f514984c`) — the merge cleanly imported upstream's new `## Git Remotes & PRs` section into `AGENTS.md`, which describes *fazer-ai's* clone: `origin` → `fazer-ai/chatwoot`, a `chatwoot-pro` remote we do not have, and a runnable `gh repo set-default fazer-ai/chatwoot`. In this clone `origin` is `Click2-Run/chatwoot` and both `fazerai` and `upstream` are `no_push`. Because `CLAUDE.md` is a **symlink** to `AGENTS.md`, the wrong topology was being loaded as authoritative agent memory — and `remote.origin.gh-resolved` was unset, so an agent following the imported text would have pinned `gh` at a third-party repo and attempted to open PRs publishing this fork's diff there. That is a disclosure risk, not merely a broken command. Section rewritten for our real remotes and marked keep-ours; `gh-resolved=base` set locally. Also added a "Fork context" preamble pointing at the `CUSTOM-*.md` family, since upstream overwrites this file on every upgrade. **No conflict fired for any of this** — the merge was clean; only reading the `AGENTS.md` diff caught it.
- **fix(whatsapp): hide session-import CTA on propriacloud inboxes** (`58a86f4f`) — upstream's new "QR code not working? import an already-linked session" block was gated only on `connection !== 'open'`, so it rendered for every provider. Session import is baileys-only by design (`import_session` exists solely on the baileys provider service; `inboxes_controller.rb:118` rejects everything else with 422), so a Própria Cloud user was being offered an action that cannot succeed and pointed at a third-party-branded browser extension to perform it. Gated on the existing `isPropriacloud` computed; baileys / zapi / whatsmeow behavior unchanged. Grep-based sweeps could not have caught this — the defect was an *absent* condition, not a wrong string.
- **docs(merges): document upgrade gaps found in the `.88` ritual run** (`2ba612ba`) — fixed the broken step-0 fetch one-liner (`git fetch fazerai upstream origin --tags` reads the trailing names as refspecs and fails; needs `--multiple`) and the stale 4.13.0 `/health` example. Added: the docker-exec prefix for all Ruby and the vite-container requirement for JS tests (host `node_modules` is stale — vitest dies on `@rollup/plugin-yaml`); a new **step 4b** covering post-merge migrations and the two false diffs `db:migrate` produces here (`schema.rb` reformatting the provider-connection index predicate, and the annotator stripping `kanban_task_id` from `conversation.rb` because this dev DB has no kanban tables); a new **step 4a** on semantic drift — the regression class the grep sweep structurally cannot catch, with a keep-ours list; tag-selection rules; and the known-failing spec baseline so the pt_BR locale failure in `inboxes_controller_spec` is not re-investigated as a merge regression. `CUSTOM-FAZER-AI.md` records the Chrome Web Store extension URL as do-not-translate.

## 2026-06

### 2026-06-28 (Upstream `.86` (Chatwoot 4.15.1) minor bump — merge + lockfile repair + upgrade-procedure fix — `39bc729a` / `31c1bd51` / `f9831a8d`)

- **Merge upstream `v4.15.1-fazer-ai.86` — minor core bump (4.14 → 4.15)** (`39bc729a`) — new branch `codi-v4.15.1-fazer-ai.86` cut **from the current `codi-v4.14.0-fazer-ai.74`** (base = our codi, so our customization history stays the `--first-parent` mainline) with the upstream tag merged INTO it — 178 upstream commits absorbed. 11 conflicts resolved per the `sync-fork` KC/AI/CO framework + `CUSTOM-FAZER-AI.md`: **`config/features.yml`** kept our `channel_whatsapp_propriacloud` at FlagShihTzu bit 3 and appended upstream's new `conversation_unread_counts` at the END of the list — inserting it at upstream's position (after `channel_facebook`) would have shifted every existing bit index and corrupted `account.feature_flags`; **`app/models/channel/whatsapp.rb`** kept our merge-semantics `update_provider_connection!` (a bare replace wipes sibling axes `is_paired`/`pair_state`/`qr_data_url` on partial updates) and layered in upstream's no-op guard, `Inbox.no_touching`, `broadcast_provider_connection_updated`, plus the new `update_reachout_time_lock!` / `update_new_chat_cap!` + `NEW_CHAT_CAP_KEYS`, keeping `whatsmeow`/`propriacloud` in `PROVIDERS`; **`sessions_controller.rb`** combined our `check_default_auth_disabled` (AUTH_DISABLE_DEFAULT) with upstream's new session-limit suite (`enforce_session_limit_for_password_login`, `track_user_session`, 409 picker); **`login/Index.vue`**, **`MessagesView.vue`**, **`ChannelItem.vue`**, **`ConversationHeader.vue`** were unions of our Propria Cloud UI (OIDC button, two-axis offline banner, `usePropriacloudLiveStream`, status badge) with upstream's reachout-restriction + message-cap banners and the `whatsapp_call` channel; **`es/inboxMgmt.json`** merged both key sets (deduped `UPDATE_PRE_CHAT_FORM_SETTINGS`); **`package.json`** + **`pnpm-lock.yaml`** took upstream's vite 6.4.2 toolchain (resolved `package.json` identical to upstream's); **`enterprise/app/models/company.rb`** was an annotate-comment-only accept. Auto-merge semantic-breakage sweep clean — no fork refs to the 8 files upstream deleted/moved (the 4.15.0 `shared/components/emoji/EmojiInput.vue` rename footgun, `app/services/whatsapp/token_validation_service.rb`, `FloatingCallWidget.vue` moved to `components-next/call/`, `working_hours_controller` folded into `inboxes_controller`). `db/schema.rb` auto-merged clean (version `2026_06_12_000000`, `f_unaccent` + propriacloud GIN index preserved, kanban-free). Post-merge rebrand sweep returned only intentional keepers (legacy-naming comments, the `fazer-ai/chatwoot` + `fazer-ai/whatsapp-api` repo URLs, the deprecated `channel_whatsapp_click2run` slot); `platform: propriacloud` intact across `health_controller`, `globals.js`, `BuildInfo.vue`, `SidebarProfileMenu.vue`, `UpdateBanner.vue`. Verified by a 4-lens read-only subagent panel (schema / per-file / high-risk / semantic-sweep — all PASS) and a live dev boot (`/health` → `{"status":"woot","platform":"propriacloud","version":"4.15.1"}`; providers + FlagShihTzu bits resolve for both our flag and the appended one). Safety tag: `codi-pre-upgrade-2026-06-27`.
- **fix(deps): repair Gemfile.lock dropped by the `.86` merge** (`31c1bd51`) — the 3-way auto-merge of `Gemfile.lock` (it auto-merged rather than conflicting) silently dropped our entire `omniauth_openid_connect` dependency subtree (`openid_connect`, `json-jwt`, `rack-oauth2`, `swd`, `webfinger`, `validate_url`, `aes_key_wrap`, `attr_required`, `bindata`) and left `vite_ruby` at 3.10.2 — violating the Gemfile's `~> 3.7.0` pin. Dev boot survived because the entrypoint runs non-frozen `bundle install`, but `bundle install --frozen` (CI / production) would have failed with the lock out of sync. Regenerated so the lock matches the Gemfile again.
- **chore(deps): follow upstream `vite_ruby` 3.10.2** (`f342091c`) — per the fork policy of tracking upstream dependency versions, dropped our `vite_ruby '~> 3.7.0'` Gemfile pin so it resolves to upstream's 3.10.2 — the matched set with the vite 6.4.2 / vite-plugin-ruby 5.2.1 npm toolchain taken in the merge. The pin's old rationale (vite-plugin-ruby 5.2.x being ESM-only vs a "CommonJS" `vite.config.ts`) no longer applies — `vite.config.ts` is ESM. `zeitwerk` held at upstream's 2.7.5 (conservative update). Verified: vite v6.4.2 boots clean, rails renders vite helpers (login 200).
- **docs(merges): standardize upgrade direction + checkpoint-first** (`f9831a8d`) — corrected `CUSTOM-MERGES-GUIDE.md` to document the upgrade direction unambiguously (new branch cut FROM the current `codi-*`, named for the new upstream tag, with that tag merged INTO it — base = our codi; AI = accept upstream tag, KC = keep our customization) and added the "commit everything as a checkpoint first" + "fetch all remotes" steps to §0. Upgrades through `.74` used the opposite base; standardized to base = our codi from `.86` onward.
- Checkpoint commits (clean tree before the merge): **`e4962ef5`** feat(whatsapp-propriacloud) return embedded-signup `expires_at` to client; **`7c5f4b88`** chore(env) sync `.env.example` branding + canonical `PROPRIACLOUD_*` vars; **`811530cd`** chore(agents) add `AGENTS.md` pointer files.

## 2026-05

### 2026-05-20 (WABA mode for Própria Cloud provider + upstream `.74` (Chatwoot 4.14) merge — `4ea94ee8` / `312e1145` / `219329d4`)

- **feat(whatsapp-propriacloud): add WABA (official Meta API) connection mode** (`4ea94ee8`) — single Própria Cloud provider card now exposes two connection types via `provider_config['connection_type']`: `web` (existing QR / phone-code pairing, whatsmeow under the hood, defaults for any legacy row) and `waba` (official Meta WhatsApp Business API instance brokered by the whatsapp-api tech-provider forwarder). Backend changes: new `WhatsappPropriacloudService#waba_mode?` helper, `setup_channel_provider` now sends `waba: waba_mode?` + forwards `phone_number_id` / `business_account_id` only when WABA, QR-pull fallback suppressed in WABA mode, `pair_qrcode` and `request_phone_pairing_code` short-circuit with a clear error, `validate_provider_config?` requires Meta credentials in WABA mode, and `Channel::Whatsapp#apply_propriacloud_defaults` defaults `connection_type` to `web` while `ensure_webhook_verify_token` skips auto-gen of `instance_id` for WABA (WABA instances are keyed by Meta credentials, not by whatsmeow UUID). Frontend: single radio "Tipo de conexão" at the top of `PropriacloudWhatsapp.vue` picks Web (Emparelhamento) vs API (Business API); the `instance_id` advanced field swaps out for `phone_number_id` + `business_account_id` in WABA mode, validated and pushed inside `provider_config`. New i18n keys `INBOX_MGMT.ADD.WHATSAPP.PROPRIACLOUD.{CONNECTION_TYPE,PHONE_NUMBER_ID,BUSINESS_ACCOUNT_ID}` in en + pt_BR. Specs: new `WABA mode` describe block (8 cases — setup, pair raises, validate, send_template, sync_templates) + new `propriacloud connection_type defaults` block on the model (2 cases). Drive-by: pre-existing PEND-09 `register_webhook!` list-stub regex rebuilt to tolerate webmock's alphabetical query-param normalization (`?.*scope=instance` instead of `?scope=instance`). Note: the public-link Embedded Signup handoff that brings WABA credentials into whatsapp-api server-side is out of scope for this commit — owned by `minha` (`POST /meta/{appId}/waba/signup` + `whatsapp_waba_public_signup_links` table). UI presently asks for manual paste as a fallback while the infra side wires up Option A (manual link handoff) or Option B (server-to-server link mint).
- **Merge upstream `v4.14.0-fazer-ai.74` — first Chatwoot core bump (4.13 → 4.14)** (`312e1145`) — created `codi-v4.14.0-fazer-ai.74` branch off the upstream tag, merged 159 commits of customizations from `codi-v4.13.0-fazer-ai.66`. Conflict resolutions (per the rebrand mapping in `CUSTOM-FAZER-AI.md` + per-file judgment): `Gemfile.lock` took upstream entirely (newer ai-agents/ruby_llm/json-jwt deps); `app/builders/account_builder.rb` kept our `default_account_locale` (`ENV['DEFAULT_ACCOUNT_LOCALE'] || 'pt_BR'`) AND added upstream's `custom_attributes: { 'onboarding_step' => 'account_details' }`; `app/javascript/dashboard/components/widgets/conversation/ConversationHeader.vue` kept both our `<PropriacloudStatusBadge :inbox="inbox" />` and upstream's new snooze-bullet separator; `app/javascript/dashboard/featureFlags.js` dropped the now-deprecated `QUOTED_EMAIL_REPLY` constant (upstream removed it because quoted email reply is now universal) while keeping all our channel flags (CHANNEL_ZAPI / CHANNEL_WHATSAPP_{BAILEYS,WHATSMEOW,PROPRIACLOUD} / CHANNEL_TWILIO_*); `config/features.yml` dropped `chatwoot_internal: true` on `companies` per upstream (companies feature is no longer internal) while keeping all our channel entries including the deprecated `channel_whatsapp_click2run` slot; `db/schema.rb` took upstream version `2026_05_15_000000` and the new companies columns (`contacts_count default 0 not null`, `additional_attributes`, `custom_attributes`, `last_activity_at`) added by PR #14435. Upstream WhatsApp fixes absorbed cleanly via auto-merge: `presence_subscribe_bulk` throttle (`conversations_controller.rb` action allowlist + 1/min budget), Baileys `disconnect_channel_provider` fail-open (rescue blocks added in `whatsapp_baileys_service.rb`), Embedded Signup persists `waba_id` (not Meta business_id) in `business_account_id`, Embedded Signup surfaces when converting non-cloud WhatsApp inboxes to whatsapp_cloud. Post-merge `grep -rEn "fazer[._-]?ai|Click2Run|click2run|\bc2r\b" app/ config/ lib/ public/brand-assets/` sweep returned 8 matches — all are intentional keepers (legacy-naming comments, the `lib/middleware/fazer_ai_platform_header` filename, the upstream `https://api.github.com/repos/fazer-ai/chatwoot/releases/latest` release-check URL, and the deprecated `channel_whatsapp_click2run` slot). Watchlist files (`SidebarProfileMenu.vue`, `UpdateBanner.vue`, `kanban/Index.vue`, `ProFeatureNudge.vue`, `BuildInfo.vue`, `login/Index.vue`, kanban i18n, `globals.js`, super_admin sessions ERB, `health_controller.rb`, `branding.rake`, `Whatsapp.vue`, `omniauth_callbacks_controller.rb`) all clean.
- **fix(whatsapp-propriacloud): align WABA template URLs with whatsapp-api 1.39.7 OpenAPI** (`219329d4`) — the initial WABA commit shipped pragmatic URL guesses for the two template paths ahead of cross-checking the upstream contract. Both were wrong: `/messages/send-template` does not exist (whatsapp-api `/messages/send` is universal and auto-routes by instance type — `waba: true` on the persisted instance record triggers Meta-Cloud forwarding, and a `message: { template: {...} }` envelope is the supported template shape), and `/templates` does not exist either (template listing for WABA instances lives at `/meta/waba/templates` on the Business API surface, paginated via Meta's cursor scheme `paging.cursors.after`). `send_template` now POSTs to `/messages/send` with the template wrapped under `message.template` (Meta-shape `name` / `language.code` / `components` preserved); `sync_templates` and the new private `fetch_waba_templates_paginated` walk `/meta/waba/templates?limit=100&after=…` accumulating all pages before a single channel update. Specs updated for the new endpoints + new case for multi-page cursor traversal. Cross-reference: `../whatsapp-api.git.worktrees/develop/docs/openapi.yaml` (dual-path routing description on `/messages/send`, schema at `/meta/waba/templates`).

### 2026-05-15 (Webhook re-registration log noise + PEND-09 spec repair)

- **fix(whatsapp-propriacloud): treat `/webhooks` 409 as idempotent BEFORE logging an error** — `register_webhook!` already handled 409 (the whatsapp-api response when a webhook with the same `(scope, instance_id, url)` triple is already registered) by falling through to `sync_webhook_subscription!`. But the condition was `process_response(response) || response.code == 409`, and `process_response` has the side effect of logging `whatsapp-api error: 409 - …` at ERROR level before returning false. Result: every `refresh_provider_status` poll fired by the dashboard re-POSTed the same body and produced a loud ERROR line in production logs, even though the flow was completely healthy. Reorder the check to `response.code == 409 || process_response(response)` so the 409 short-circuits ahead of the noisy logger call. Reconcile semantics, `sync_webhook_subscription!` dedupe sweep, and the genuine-failure raise path are all preserved. Surfaced during the multicanal-ppcloud-0 inbox-create investigation (Multicanal Própria Cloud prod, inbox `0119` / `+551431190119`): the inbox + upstream instance were created correctly, phone-code pairing reached whatsmeow, but the ERROR-level 409 log on every dashboard tick looked like a real failure to operators reading `kubectl logs`. Now the noise is gone.
- **test(whatsapp-propriacloud): repair the PEND-09 spec + add 409 / 500 / no-ERROR regressions** — the existing `register_webhook!` spec asserted `body['active'] == true` but the service no longer sends `active` (the field was removed from `CreateWebhookRequest` as part of the original PEND-09 closure; the service comment was updated but the spec was not — the test had been failing the matcher on every run). Replace the matcher with the documented contract (`!body.key?('enabled') && !body.key?('active')`), and add three sibling examples: (1) 409 CONFLICT returns success and still runs `sync_webhook_subscription!`, (2) the 409 path does NOT emit `whatsapp-api error: 409` via `Rails.logger.error`, and (3) a genuine 500 still raises `ProviderUnavailableError`. Covers both the bug above and the previously-broken spec in one block.
