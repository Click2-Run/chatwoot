...
class Channel::Whatsapp < ApplicationRecord # rubocop:disable Metrics/ClassLength
  ...
  def update_provider_connection!(provider_connection)
    ...
    # MERGE (not replace) into the existing jsonb — see the method comment above. A bare replace
    # would wipe sibling axes (is_paired, pair_state, qr_data_url, error, ...) whenever a caller
    # passes a partial hash like { connection: 'close' }. Pass nil for a key to clear it.
    merged = (self.provider_connection || {})
             .deep_dup
             .merge(provider_connection.deep_stringify_keys)
    # Normalize comparison so an unchanged status is recognized as a no-op and skipped.
    return if merged == self.provider_connection
