...
# Whatsmeow Event Handlers - Helper Methods
# Adapted from Baileys handlers for Whatsmeow event format
#
...
module Whatsapp::WhatsmeowHandlers::Helpers # rubocop:disable Metrics/ModuleLength
  ...
  def sender_lid
    # Whatsmeow doesn't use LID in the same way as Baileys
    # LID (Linked ID) is handled internally by whatsmeow
  ...
  def message_type # rubocop:disable Metrics/CyclomaticComplexity,Metrics/PerceivedComplexity,Metrics/MethodLength,Metrics/AbcSize
    ...
    # Whatsmeow uses similar message structure to Baileys
  ...
  def contact_name
    # Whatsmeow provides push_name in events
  ...
  # Whatsmeow uses Unix timestamps (int64), not Baileys' {low, high, unsigned}
