...
module Whatsapp::WhatsmeowHandlers::ConnectionUpdate
  ...
  def process_connection_update
    ...
    # Connection states from Whatsmeow:
  ...
  def extract_qr_data_url(data)
    # Whatsmeow sends QR code as base64 PNG when connecting
  ...
  def log_connection_state(data)
    ...
    if error.present?
      Rails.logger.error "Whatsmeow connection error",
                         inbox_id: inbox.id,
                         phone_number: inbox.channel.phone_number,
                         error: error
    else
      Rails.logger.info "Whatsmeow connection update",
                        inbox_id: inbox.id,
                        phone_number: inbox.channel.phone_number,
